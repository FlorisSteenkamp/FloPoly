...
function allRootsVAS(p_) {
    ...
    let vasRoots = vasRootIntervals(p)
        ...
        return brent_1.brent(evaluate_2.evaluate(p), interval[0], interval[1]);
    });
...
function removeZeroRoots(p_) {
    let p = p_.slice();
    let i = 0;
    while (evaluate_at_0_2.evaluateAt0(p) === 0) {
        let len = p.length;
        p.pop();
        i++;
    }
...
function vasRootIntervals(p) {
    ...
    let negativeIntervals = vasRootIntervalsHelper(change_variables_linear_1.changeVariablesLinear(p.slice(), -1, 0), [[1, 0], [0, 1]])
        .map(function (interval) {
        return negate_1.negate(invert_2.invert(interval));
    });
...
function vasRootIntervalsHelper(p, mobius) {
    ...
    // STEP 1
    let intervals = [];
    let signVariations = sign_changes_1.signChanges(p);
    // STEP 2
    ...
    // STEP 3
    if (signVariations === 1) {
        let M0 = evaluate_at_0_1.mobEvaluateAt0(mobius);
        let MI = evaluate_at_inf_1.mobEvaluateAtInf(mobius);
        let MM0 = Math.min(M0, MI);
        let MMI = Math.max(M0, MI);
        if (MMI === Number.POSITIVE_INFINITY) {
            MMI = evaluate_1.mobEvaluate(mobius, root_bounds_lmq_1.positiveRootUpperBound_LMQ(p));
        }
        return [[MM0, MMI]];
    }
    // STEP 4
    let lb = root_bounds_lmq_1.positiveRootLowerBound_LMQ(p);
    // STEP 5
    if (lb > 1) {
        // p ← p(x + lb)
        p = change_variables_linear_1.changeVariablesLinear(p, 1, lb);
        // M ← M(x + lb)
        mobius = change_variables_1.mobChangeVariables(mobius, 1, lb);
    }
    // TODO - Include factor of 16 improvement by Strzebonski
    // STEP 6 - Look for real roots in (0, 1)
    // p01 ← (x + 1)^(deg(p)) *  p(1/(x+1))
    let p01 = change_variables_linear_1.changeVariablesLinear(invert_2.invert(p), 1, 1);
    // M01 ← M(1/(x+1))
    let M01 = change_variables_1.mobChangeVariables(invert_1.mobInvert(mobius), 1, 1);
    // STEP 7 - Is 1 a root?
    let m = evaluate_1.mobEvaluate(mobius, 1);
    // STEP 8 - Look for real roots in (1, ∞)
    // p1∞ ← p(x + 1)
    let p1inf = change_variables_linear_1.changeVariablesLinear(p, 1, 1);
    // M1∞ ← M(x + 1)
    let M1inf = change_variables_1.mobChangeVariables(mobius, 1, 1);
    // STEPS 9 -> 13
    let intervals1 = vasRootIntervalsHelper(p01, M01);
    let intervals3 = vasRootIntervalsHelper(p1inf, M1inf);
    if (evaluate_2.evaluate(p)(1) === 0) {
        intervals1.push([m, m]);
    }
