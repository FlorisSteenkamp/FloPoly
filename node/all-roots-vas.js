...
function allRootsVAS(p_) {
    ...
    let vasRoots = vasRootIntervals(p)
        ...
        return root_operators_1.brent(core_operators_1.evaluate(p), interval[0], interval[1]);
    });
...
function removeZeroRoots(p_) {
    let p = p_.slice();
    let i = 0;
    while (core_operators_1.evaluateAt0(p) === 0) {
        let len = p.length;
        p.pop();
        i++;
    }
...
function vasRootIntervals(p) {
    ...
    let negativeIntervals = vasRootIntervalsHelper(core_operators_1.changeVariables(p.slice(), -1, 0), [[1, 0], [0, 1]])
        .map(function (interval) {
        return core_operators_1.negate(core_operators_1.invert(interval));
    });
...
function vasRootIntervalsHelper(p, mobius) {
    ...
    // STEP 1
    let intervals = [];
    let signVariations = core_operators_1.signChanges(p);
    // STEP 2
    ...
    if (signVariations === 1) {
        ...
        if (MMI === Number.POSITIVE_INFINITY) {
            MMI = mobius_1.default.evaluate(mobius, root_bounds_1.positiveRootUpperBound_LMQ(p));
        }
        return [[MM0, MMI]];
    }
    // STEP 4
    let lb = root_bounds_1.positiveRootLowerBound_LMQ(p);
    // STEP 5
    if (lb > 1) {
        // p ← p(x + lb)
        p = core_operators_1.changeVariables(p, 1, lb);
        // M ← M(x + lb)
    ...
    let p01 = core_operators_1.changeVariables(core_operators_1.invert(p), 1, 1);
    // M01 ← M(1/(x+1))
    ...
    // p1∞ ← p(x + 1)
    let p1inf = core_operators_1.changeVariables(p, 1, 1);
    // M1∞ ← M(x + 1)
    ...
    if (core_operators_1.evaluate(p)(1) === 0) {
        intervals1.push([m, m]);
    }
