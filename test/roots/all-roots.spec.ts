...
import { rootsAccurateEnough } from '../helpers/roots-accurate-enough';
import { numsWithin } from '../helpers/nums-within';
import { 
	allRoots, fromRoots,
	flatRootsArr, flatCoefficientsArr, flatCoefficients
} from '../../src/index';
...
describe('allRoots', function() {
	...
	function() {
		...
		// roots at -1, 0.1, 0.2
		let roots = allRoots(p, undefined, 0);
		assert(
			roots.length === 1 &&
	...
	function() {
		let p = [1, 0.7, -0.28, 0.02];
		// roots at -1, 0.1, 0.2
		let roots = allRoots(p, 0, undefined);
		assert(
			roots.length === 2 &&
