...
function bExactDiv(a: bigint[], b: bigint[]): bigint[] {
    if (b.length === 0) {
...
function bGcdPrsSafe(a: bigint[], b: bigint[]): bigint[] {
 ...
 */
function bYunsAlgorithm(
    p: bigint[]): { factor: bigint[]; multiplicity: number; }[] {
    ...
    const factors: { factor: bigint[]; multiplicity: number; }[] = [];
