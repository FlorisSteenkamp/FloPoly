 ...
 * 
 * @param pDd_ defaults to `undefined`; an error polynomial that provides a 
 * coefficientwise error bound on the input polynomial with the actual error bound
 * being `Eᵢ * γγ(3)` for the coefficient of `xⁱ`
 * all coefficients must be positive; if `undefined `then the input polynomial
 * will be assumed exact
 * 
 ...
 * const { pDd, pE, getPExact } = eFromRoots(_roots);  
 ...
 * const rs = roots(pDd,0,51,pE,getPExact);
 * console.log(rs);  // => [
 ...
 * //
 * // Note: Due to floating point underflow/overflow of the evaluation of a
 * // Wilkinson polynomial of degree >= 58 evaluated at 59 the returned roots
 * // starts getting inaccurate at this degree (i.e. >= 58).
 * ```
 * 
 * @doc
 */
function roots(
    ...
    // `pDd`, `EsDd` and `getPExact_()` *must* be of same length
    //----------------------------------------------------------------------
    // TODO - improve this piece of code - it's ugly af
    // ({ pDd, EsDd, getPExact_ } = removeLeadingZeroCoeffsUsingEs(pDd, EsDd, getPExact_));
    // TODO - put back
    // {
    //     const { p: p_, pE, pExact } = removeLeadingZeroCoeffs(pDd, EsDd, getPExact_);
    //     if (pExact !== undefined) {
    //         getPExact_ = () => pExact;
    //         pDd = p_;
    //         EsDd = pE;
    ...
    const Is = isolateRoots(p, pDd, pDd_, lb, ub, getPExact_);
    if (Is === undefined) {
        // bail out to a different root isolation method for very hard cases
        // console.log('bailed');
        return allRootsCertified(pDd, lb, ub, pDd_, getPExact_, true);
    }
