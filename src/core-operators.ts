 ...
 */
function changeVariables(p: number[], a: number, b: number): number[] {
 ...
 */
function clip0(p: number[]): number[] {
 ...
 */
function maxCoefficient(p: number[]): number {
	let max = 0;
 ...
 */
function toCasStr(p: number[]): string {
	let d = p.length-1;
