 ...
 */
function gcdIntBinary(a: number, b: number): number {
	a = Math.abs(a);
	b = Math.abs(b);
 ...
 */
function gcdIntsTree(vals: number[]) {
	let vals_ = vals.slice();

    // make array of numbers all positive
	for (let i=0; i<vals_.length; i++) { vals_[i] = Math.abs(vals_[i]); }
