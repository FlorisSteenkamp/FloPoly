...
import { taylorShiftWithInpErr } from '../../../src/change-variables/double/taylor-shift-with-inp-err.js';
import { eeTaylorShift } from '../../../src/change-variables/expansion/ee-taylor-shift.js';

const { abs } = Math;


test('`taylorShiftWithInpErr`', function() {
    {
        const N = 100;
        const shift = 0;
        ...
        for (let i=0; i<N; i++) {
            ...
            // toCasStr(pEe);//?
            const r0 = 0.47;
            const vsEe = eeTaylorShift(polyExp,[r0]).map(eCompress);

            const [r̂,r̂_] = taylorShiftWithInpErr(polyPert, r0, errBound);
            r̂;
            r̂_;
