import { getImplicitD } from "./get-implicit-d.js";
import { toPowerBasis3WithRunningError } from "./to-power-basis-with-running-error.js";
import { γ1 } from '../../../../src/error-analysis/gamma.js';

const { abs } = Math;


function getCoeffsD(ps1: number[][], ps2: number[][]) {
    const { 
    ...
    const {
        ...
        errorBound: [[c3_,c2_,c1_],[d3_,d2_,d1_]]  // c0 and d0 are exact, so no error 
    ...
    const c0c2_ = _c0*c2_ + _c0c2;
    const c0c3_ = _c0*c3_ + _c0c3;
    const c0d0_ = _c0d0;
    const c0d1_ = _c0*d1_ + _c0d1;
    const c0d2_ = _c0*d2_ + _c0d2;
    const c0d3_ = _c0*d3_ + _c0d3;
    const c1c1_ = 2*(_c1*c1_) + _c1c1;
    const c1c2_ = c1_*_c2 + _c1*c2_ + _c1c2;
    const c1c3_ = c1_*_c3 + _c1*c3_ + _c1c3;
    const c1d0_ = c1_*_d0 + _c1d0;
    const c1d1_ = c1_*_d1 + _c1*d1_ + _c1d1;
    const c1d2_ = c1_*_d2 + _c1*d2_ + _c1d2;
    const c1d3_ = c1_*_d3 + _c1*d3_ + _c1d3;
    const c2c2_ = 2*(_c2*c2_) + _c2c2;
    const c2c3_ = c2_*_c3 + _c2*c3_ + _c2c3;
    const c2d0_ = c2_*_d0 + _c2d0;
    const c2d1_ = c2_*_d1 + _c2*d1_ + _c2d1;
    const c2d2_ = c2_*_d2 + _c2*d2_ + _c2d2;
    const c2d3_ = c2_*_d3 + _c2*d3_ + _c2d3;
    const c3c3_ = 2*(c3_*_c3) + _c3c3;
    const c3d0_ = c3_*_d0 + _c3d0;
    const c3d1_ = c3_*_d1 + _c3*d1_ + _c3d1;
    const c3d2_ = c3_*_d2 + _c3*d2_ + _c3d2;
    const c3d3_ = c3_*_d3 + _c3*d3_ + _c3d3;
    const d0d0_ = _d0d0;
    const d0d1_ = _d0*d1_ + _d0d1;
    const d0d2_ = _d0*d2_ + _d0d2;
    const d0d3_ = _d0*d3_ + _d0d3;
    const d1d1_ = 2*(_d1*d1_) + _d1d1;
    const d1d2_ = d1_*_d2 + _d1*d2_ + _d1d2;
    const d1d3_ = d1_*_d3 + _d1*d3_ + _d1d3;
    const d2d2_ = 2*(_d2*d2_) + _d2d2;
    const d2d3_ = d2_*_d3 + _d2*d3_ + _d2d3;
    const d3d3_ = 2*(_d3*d3_) + _d3d3;
    ...
    const g1 = c3*c3c3;
    const _g1 = abs(g1);
    const g1_ = c3_*_c3c3 + _c3*c3c3_ + abs(g1);
    const g2 = c3*d3d3;
    const _g2 = abs(g2);
    const g2_ = c3_*_d3d3 + _c3*d3d3_ + abs(g2);
    const g3 = d3*c3c3;
    const _g3 = abs(g3);
    const g3_ = d3_*_c3c3 + _d3*c3c3_ + abs(g3);
    const g4 = d3*d3d3;
    const _g4 = abs(g4);
    const g4_ = d3_*_d3d3 + _d3*d3d3_ + abs(g4);
    ...
    const u5 = 3*(w7 + u1);
    const u5_ = 3*(w7_ + u1_ + abs(w7 + u1)) + abs(u5);
    const v8 = u4 + u5;
    ...
    const wn = 3*(wk + wl);
    const wn_ = 3*(wk_ + wl_ + abs(wk + wl)) + abs(wn);
    const v7 = wm + wn;
    ...
    const zc = d2*c2c2;
    const _zc = abs(zc);
    const zc_ = d2_*_c2c2 + _d2*c2c2_ + _zc;
    const zd = 2*c1*wo;
    ...
    const wz = (3*c3)*wy;
    const _wz = abs(wz);
    const wz_ = (3*c3_ + abs(3*c3))*_wy + abs(3*c3)*wy_ + _wz;
    const z1 = wx + wz;
    const _z1 = abs(z1);
    const z1_ = wx_ + wz_ + _z1;
    const z2 = d2*d2d2;
    const _z2 = abs(z2);
    const z2_ = d2_*_d2d2 + _d2*d2d2_ + _z2;
    const z3 = 2*d1d2 + d0d3;
    const _z3 = abs(z3);
    const z3_ = 2*d1d2_ + d0d3_ + _z3;
    const z4 = (3*d3)*z3; 
    const _z4 = abs(z4);
    const z4_ = (3*d3_ + abs(3*d3))*_z3 + abs(3*d3)*z3_ + _z4;
    const z5 = z2 + z4;
    const _z5 = abs(z5);
    ...
    const zm = vₓₓ*c3c3;
    const zm_ = vₓₓ_*_c3c3 + _vₓₓ*c3c3_ + abs(zm);
    const zn = vᵧᵧ*d3d3;
    const zn_ = vᵧᵧ_*_d3d3 + _vᵧᵧ*d3d3_ + abs(zn);
    const z8 = zm + zn;
    const z8_ = zm_ + zn_ + abs(z8);
    const z9 = vₓᵧ*c3d3;
    const z9_ = vₓᵧ_*_c3d3 + _vₓᵧ*c3d3_ + abs(z9);
    const za = z6 + z7;
    ...
    const kv = 3*(ks + kt);
    const _kv = abs(kv);
    const kv_ = 3*(ks_ + kt_ + abs(ks + kt)) + _kv;
    const kw = vₓᵧ*wo;
    ...
    const sb = c2*c1c1;
    const _sb = abs(sb);
    const sb_ = c2_*_c1c1 + _c2*c1c1_ + _sb;
    const sc = d2*d1d1;
    const _sc = abs(sc);
    const sc_ = d2_*_d1d1 + _d2*d1d1_ + _sc;
    const sd = s1 + s3;
    ...
    const r8 = c1c2 + c0c3;
    const _r8 = abs(r8);
    const r8_ = c1c2_ + c0c3_ + _r8;
    const r9 = d1d2 + d0d3;
    const _r9 = abs(r9);
    const r9_ = d1d2_ + d0d3_ + _r9;
    ...
    const m5 = r8 + c1c2;
    const _m5 = abs(m5);
    const m5_ = r8_ + c1c2_ + _m5;
    const m6 = r9 + d1d2;
    const _m6 = abs(m6);
    const m6_ = r9_ + d1d2_ + _m6;
    const m7 = (3*c0)*m5;
    const m7_ = abs(3*c0)*(_m5 + m5_) + abs(m7);
    const m8 = (3*d0)*m6;
    const m8_ = abs(3*d0)*(_m6 + m6_) + abs(m8);
    const m9 = c1*c1c1;
    const _m9 = abs(m9);
    const m9_ = c1_*_m9 + _c1*c1c1_ + abs(m9);
    const ma = d1*d1d1;
    const _ma = abs(ma);
    const ma_ = d1_*_ma + _d1*d1d1_ + abs(ma);
    const mb = vₓₓ*r8;
    ...
    const ra = c1d1 + c2d0;
    const _ra = abs(ra);
    const ra_ = c1d1_ + c2d0_ + _ra;
    const rb = c1d1 + c0d2;
    const _rb = abs(rb);
    const rb_ = c1d1_ + c0d2_ + _rb;
    ...
    const l1 = 2*ra + c0d2;
    const _l1 = abs(l1);
    const l1_ = 2*ra_ + c0d2_ + _l1;
    const l2 = 2*rb + c2d0;
    const _l2 = abs(l2);
    const l2_ = 2*rb_ + c2d0_ + _l2;
    const l3 = c0*l1;
    const _l3 = abs(l3);
    const l3_ = _c0*l1_ + _l3;
    const l4 = d0*c1c1;
    ...
    const l8 = c2*c0c0;
    const l8_ = c2_*_c0c0 + _c2*c0c0_ + abs(l8);
    const l9 = d0*d1d1;
    const l9_ = _d0*d1d1_ + abs(l9);
    const la = d2*d0d0;
    const la_ = d2_*_d0d0 + _d2*d0d0_ + abs(la);
    const lb = l3 + l4;
    const _lb = abs(lb);
    ...
    const lh = 3*(lf + lg);
    const lh_ = 3*(lf_ + lg_ + abs(lf + lg)) + abs(lh);
    const li = ra + c0d2; 
    const _li = abs(li);
    ...
    const rx = c1*c0c0;
    const _rx = abs(rx);
    const rx_ = c1_*_c0c0 + _c1*c0c0_ + _rx;
    const rh = rx*vₓₓₓ;
    const rh_ = rx_*_vₓₓₓ + _rx*vₓₓₓ_ + abs(rh);
    const ry = d1*d0d0;
    const _ry = abs(ry);
    const ry_ = d1_*_d0d0 + _d1*d0d0_ + _ry;
    const ri = ry*vᵧᵧᵧ;
    ...
    const rk = c0c1*vₓₓ;
    const _rk = abs(rk);
    const rk_ = c0c1_*_vₓₓ + _c0c1*vₓₓ_ + _rk;
    const rl = d0d1*vᵧᵧ;
    const _rl = abs(rl);
    const rl_ = d0d1_*_vᵧᵧ + _d0d1*vᵧᵧ_ + _rl;
    const rm = rk + rl;
    ...
    const ro = d1*vᵧ;
    const _ro = abs(ro);
    const ro_ = d1_*_vᵧ + _d1*vᵧ_ + _ro;
    const rp = rn + ro;
    const rp_ = rn_ + ro_ + abs(rp);
    const rq = rd*rf;
    const _rq = abs(rq);
    const rq_ = rd_*_rf + _rd*rf_ + _rq;
    const rr = re*rg;
    const _rr = abs(rr);
    const rr_ = re_*_rg + _re*rg_ + _rr;
    const rs = rq + rr;
    const rs_ = rq_ + rr_ + abs(rs);
    const rt = 3*(rh + ri);
    const rt_ = 3*(rh_ + ri_ + abs(rh + ri)) + abs(rt);
    const ru = rj + 2*rm;
    ...
    const pc = c0c0*p7;
    const pc_ = c0c0_*_p7 + _c0c0*p7_ + abs(pc);
    const pd = d0d0*p8;
    const pd_ = d0d0_*_p8 + _d0d0*p8_ + abs(pd);
    const p6 = pc + pd;
    const p6_ = pc_ + pd_ + abs(p6);
    const pe = c0d0*vₓᵧ;
    const pe_ = c0d0_*_vₓᵧ + _c0d0*vₓᵧ_ + abs(pe);
    const p9 = p6 + pe;
    ...
    return {
        ...
        // should still be multiplied by γ1
